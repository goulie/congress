@if (session()->has('success'))
    <div class="alert alert-success " role="alert">
        <div class="d-flex align-items-center">
            ...
            <strong>{{ app()->getLocale() == 'fr' ? 'Succès !' : 'Success!' }}</strong>
            <div class="mt-1">
                {{ session('success') }}
            </div>
        </div>
...
<form method="POST" action="{{ route('add.accompagning.form') }}" enctype="multipart/form-data">
    @csrf
    ...
    <div class="box-body">
        ...
        <div class="row">
            ...
            <div class="col-md-4">
                <label class="control-label font-weight-bold font-weight-bold text-dark">
            ...
            <div class="col-md-4">
                <label class="control-label font-weight-bold text-dark font-weight-bold">
            ...
            <div class="col-md-4">
                ...
                <select class="form-control @error('category') is-invalid @enderror" name="type_accompanying"
                    ...
                    @forelse (App\Models\AccompanyingPersonType::get()->translate(app()->getLocale(), 'fallbackLocale') as $category)
                        <option data-amount="{{ $accompanying->montant }}" value="{{ $category->id }}"
                            {{ old('type_accompanying') == $category->id ? 'selected' : (session('participant_data.type_accompanying') == $category->id ? 'selected' : '') }}>
                            {{ $category->libelle }}
                        </option>
                    @empty
                        <option disabled>{{ __('registration.no_data') ?? 'No data' }}</option>
            ...
            <div class="col-md-8 text-center">
                <div class="alert text-center">
                    <p>
                        <span
                            style="font-size: 1.5em;font-weight: bold;color:black">{{ app()->getLocale() == 'fr' ? 'Montant à payer' : 'Amount to pay' }}
                            :</span><br>
...
<div class="panel panel-default panel-custom" style="padding-top: 50px">
    ...
    <div class="panel panel-default">
        <div class="panel-heading clearfix bg-white" style="background-color: #fff">
            <h4 class="pull-left">
        ...
        <table class="table table-striped table-hover table-condensed">
            <thead>
                <tr>
                    ...
                    <th>Email</th>
                    <th>{{ app()->getLocale() == 'fr' ? 'type de personne' : 'type of person' }}</th>
                ...
                @forelse ($participants as $participant)
                    ...
                    <tr>
                        ...
                        <td>
                            <span class="label label-dark" style="font-size:14px">
                                ...
                                @elseif ($congres->currency == 'US' || $congres->currency == 'USD')
                                    {{ '$ ' . $invoice->total_amount }}
                                @else
                                    {{ $invoice->total_amount . ' ' . $congres->currency }}
                                @endif
                            </span>
                        </td>
                        ...
                        <td class="text-center">
                            @php
                                switch ($invoice->participant->status) {
                                    case 'paid':
                                    case 'Payé':
                        ...
                        <td class="text-right">
                            ...
                            <a href="{{ route('accompagning.edit', $participant->uuid ?? '') }}"
                                class="btn btn-warning btn-xs">
                                <i class="glyphicon glyphicon-edit"></i>
                                {{ app()->getLocale() == 'fr' ? 'Modifier' : 'Edit' }}
                            </a>
                            <button class="btn btn-danger btn-xs delete-participant"
                                            ...
                                            <div class="col-md-6">
                                                <p class="mb-2">
                                                    ...
                                                    <span id="detail-total-amount"
                                                        class="h5 text-success ml-2"></span>
                                                </p>
                                            </div>
                                            <div class="col-md-6">
                                                <p class="mb-2">
