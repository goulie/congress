@php
    ...
    $isFr = $participant->langue === 'fr';
    $paid = $invoice->status === App\Models\Invoice::PAYMENT_STATUS_PAID;
    $deadline = now()->startOfMonth()->addDays(11)->format('d/m/Y');
@endphp
...
===================================================== --}}
    @if ($paid)
# {{ $isFr ? 'Votre facture a été réglée' : 'Your invoice has been paid' }}
    @else
# {{ $isFr ? 'Votre facture est disponible' : 'Your invoice is available' }}
    @endif
...
<tr>
    <th align='left'>{{ $isFr ? 'Désignation' : 'Description' }}</th>
    <th align='right'>{{ $isFr ? 'Montant' : 'Amount' }}</th>
</tr>
...
@endforeach
<tr>
    <td><strong>{{ $isFr ? 'Total' : 'Total Amount' }}</strong></td>
